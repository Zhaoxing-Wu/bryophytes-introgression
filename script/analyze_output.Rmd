...
tree = root(tree,"Anthocerotales", resolve.root = TRUE) #reroot on the outgroup
```
...
displays the heatmap of the proportion of times that each taxon is involved in a significant hybridization event
as identified by MSCquartets, HyDe, Patterson’s D-statistics and D3
...
Figure 1: Left: Phylogenetic tree of selected species from the Bryophyta group. Right: Heatmap of frequency of taxa identified as part of hybridization events (scaled between 0 and 1 for comparison across methods). Tests are performed at the significance level of 0.05. The grey tile means that the taxa is missing for its corresponding algorithm.

```{r, message=FALSE, warning=FALSE, echo=FALSE,fig.width = 16, fig.height=8,fig.align='center'}
p_tree + p_bonferroni
```

Figure 2: Left: Phylogenetic tree of selected species from the Bryophyta group. Right: Heatmap of frequency of taxa identified as part of hybridization events (scaled between 0 and 1 for comparison across methods). Tests are Bonferroni-corrected. The grey tile means that the taxa is missing for its corresponding algorithm.
